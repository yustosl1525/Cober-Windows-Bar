...
describe("desktopStatusInputRuntime.test", () => {
  const fixtureEvents = [
    {
      id: "fixture-download",
  ...
  async function testDesktopStatusRuntimeSubscribersReceiveBusUpdates() {
    ...
    assert.deepEqual(observedEventIds[observedEventIds.length - 1], [
      "live-ai",
      "fixture-download",
    ]);
  ...
  async function testDesktopStatusRuntimeAcceptsPushListenerUpdates() {
    let emit:
      | ((events: typeof fixtureEvents, source?: "mock" | "tauri-fixture") => void)
      | undefined;
    const runtime = createDesktopStatusRuntime({
      fallbackEvents: fixtureEvents,
  ...
  async function testTauriDesktopStatusEventSourceEmitsCanonicalEvents() {
    ...
    let handler: ((event: { payload: unknown }) => void | Promise<void>) | undefined;
  ...
  it("testFallsBackToMockWithoutInvoke", async () => {
    await testFallsBackToMockWithoutInvoke();
  });
  it("testLoadsFixtureEventsFromTauriInvoke", async () => {
    await testLoadsFixtureEventsFromTauriInvoke();
  });
  it("testSkipsFixtureLoadWhenDisabled", async () => {
    await testSkipsFixtureLoadWhenDisabled();
  });
  it("testFallsBackToMockWhenFixtureLoadFails", async () => {
    await testFallsBackToMockWhenFixtureLoadFails();
  });
  it("testDesktopStatusRuntimeCanStartFromResidentProductInput", async () => {
    await testDesktopStatusRuntimeCanStartFromResidentProductInput();
  });
  it("testDesktopStatusRuntimeSeedsBusAndRefreshesFromFixtureSource", async () => {
    await testDesktopStatusRuntimeSeedsBusAndRefreshesFromFixtureSource();
  });
  it("testDesktopStatusRuntimeSubscribersReceiveBusUpdates", async () => {
    await testDesktopStatusRuntimeSubscribersReceiveBusUpdates();
  });
  it("testDesktopStatusRuntimeAcceptsPushListenerUpdates", async () => {
    await testDesktopStatusRuntimeAcceptsPushListenerUpdates();
  });
  it("testDesktopStatusRuntimeMarksFallbackAsStaleAfterSuccessfulNativeRefresh", async () => {
    await testDesktopStatusRuntimeMarksFallbackAsStaleAfterSuccessfulNativeRefresh();
  });
  it("testDesktopStatusRuntimeDisposeUnsubscribesPushListener", async () => {
    await testDesktopStatusRuntimeDisposeUnsubscribesPushListener();
  });
  it("testTauriDesktopStatusEventSourceEmitsCanonicalEvents", async () => {
    await testTauriDesktopStatusEventSourceEmitsCanonicalEvents();
  });
  it("testDesktopStatusRuntimeUsesTauriEventSourceAndUnlistensOnDispose", async () => {
